...
$("#restart").on("click", function () {
    location.reload();
    let result = confirm("黒板を消しますか？saveしていない黒板は消去されます。")
    if (result) {
        location.reload();
    }
...
// firebaseからのデータの取得
$("#icon_load").on("click", function () {



    // Realtimedatabaseからデータを取得、読取ファイルのselect optionを生成する
    get(child(ref(getDatabase(app)), "iconsave")).then((snapshot) => {
        if (snapshot.exists()) {
            const data = snapshot.val();
            // 配列として取り出す
            const dataArray = Array.isArray(data) ? data : Object.values(data);
            ...
            list_title.forEach(function (element) {
                ...
                <option value="${title_display}">${title_display}</option>
                `;
                $("#icon_select_title").append(add_select);
...
// 選択したtitleに対応するdataを取得する
$("#icon_data_load").on("click", function () {
    let result = confirm("選択したiconを追加しますか？")
    if (result) {
        //select optionの初期化
        // $(".outer_wrap").empty();
        const title_select = $("#icon_select_title").val();
        const title_array_index = list_title.indexOf(title_select)
        $(".outer_dragitem").append(list_data[title_array_index]);
        alert("黒板をloadしました")
    }
...
// テキストエリアが変更された時に発火し、valを更新する
$(document).on("change", "textarea", function () {
    const textchange = $(this).val();
